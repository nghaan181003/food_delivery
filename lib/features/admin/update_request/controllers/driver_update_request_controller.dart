...
class DriverUpdateRequestController extends GetxController {
  static DriverUpdateRequestController get instance => Get.find();
  ...
  void fetchAllRequest() async {
    try {
      ...
      final data = await _repository.fetchAllDriverRequest();
  ...
  Future<void> approveUpdate(String id) async {
    try {
      ...
      final isApproved = await _repository.approveDriverUpdate(id);
      if (isApproved) {
        fetchAllRequest();
  ...
  Future<void> rejectUpdateRequest(String id) async {
    try {
      final res = await _repository.rejectDriverUpdateRequest(id);
