...
class SendVoucherScreen extends StatefulWidget {
  const SendVoucherScreen({super.key, this.voucher, required this.rating});

  final VoucherModel? voucher;
  final RatingModel rating;

  @override
  State<SendVoucherScreen> createState() => _SendVoucherScreenState();
}

class _SendVoucherScreenState extends State<SendVoucherScreen> {
  ...
  void initState() {
    ...
    _name = TextEditingController(
        text: widget.voucher?.name ?? "Xin lỗi bạn vì trãi nghiệm chưa tốt");
    _code = TextEditingController(text: widget.voucher?.code);
    _quantity = TextEditingController(text: "1");
    _maxUsersPerUser = TextEditingController(text: "1");
  ...
  VoucherModel get toVoucherModel {
    return VoucherModel(
      ...
      customers: [
        VoucherCustomer(
          customerId: widget.rating.customerId,
          customerName: widget.rating.customerName,
        )
      ],
    );
  }
        ...
        children: [
          ...
          Expanded(
            child: Text(
                ...
                "Bạn có thể gửi tặng khách voucher như một sự tri ân hoặc đền bù vì trải nghiệm chưa tốt. Chỉ khách hàng này có thể sử dụng voucher để đặt đơn với Quán bạn. Giá trị voucher sẽ do quán bạn chi trả."),
          ),
        ],
      ),
    );
  ...
  Widget get _buildReceiverInfor {
    return _buildContainer(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _buidRowWithLabel(
              title: "Gửi tới", child: Text(widget.rating.customerName)),
          _buidRowWithLabel(
              title: "Mã đơn hàng", child: Text(widget.rating.orderId))
        ]
            .expand((e) => [
                  e,
    ...
    return _buildContainer(
      ...
      children: [
        _buidRowWithLabel(
            ...
            child: Expanded(
                child: MyTextFiled(
                    textController: _name, hintText: "Nhập tiêu đề"))),
        _buidRowWithLabel(
            title: "Mã voucher *",
            child: Expanded(
                child: MyTextFiled(
                ...
                children: [
                  ...
                  _buidRowWithLabel(
                    ...
                    child: Expanded(
                      child: MyTextFiled(
                        isNumberType: true,
                        symbol: value.ispercentage ? "%" : " VND",
                        useThousandsFormatter: true,
  ...
  Widget build(BuildContext context) {
    ...
    return MobileWrapper(
      appBar: CustomAppBar(
        title: Text("Tặng voucher cho khách hàng"),
      ),
          ...
          onPressed: () async {
            ...
            final result =
                ...
                await notificationController
                    .sendNotificationToVoucher(widget.rating.customerId, widget.rating.orderId);
